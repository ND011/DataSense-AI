        ...
        if chart_type == "bar":
            ...
            if map_type == "choropleth":
                ...
                if loc_col in df.columns and color_col in df.columns:
                    ...
                    # Detect if a time dimension exists to preserve it
                    time_col = next((c for c in df.columns if any(kw in c.lower() for kw in ['year', 'date', 'month', 'time'])), None)
                    if time_col:
                        temp_df[time_col] = df[time_col]
                        temp_df = temp_df.dropna(subset=[color_col])
                        # Group by location AND time to preserve the time-series
                        agg = temp_df.groupby([loc_col, time_col], observed=False)[color_col].sum().reset_index()
                    else:
                        temp_df = temp_df.dropna(subset=[color_col])
                        agg = temp_df.groupby(loc_col, observed=False)[color_col].sum().reset_index()

                    for _, row in agg.iterrows():
                        val = row[color_col]
                        if isinstance(val, (pd.Series, np.ndarray)): val = val[0]
                        if pd.notna(val):
                            item = {"location": str(row[loc_col]), "value": float(val)}
                            if time_col: 
                                # Convert to string to prevent JSON serialization errors (Timestamp objects)
                                item[time_col] = str(row[time_col])
