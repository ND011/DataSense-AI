    ...
    if (iqr > 0 && height > 0) {
        ...
        return (
            <g>
                ...
                <line x1={x} y1={y} x2={x + width} y2={y} stroke="#111221" strokeWidth={1} />
            </g>
        );
    }
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            <div className="bg-gradient-to-br from-app-accent3 to-app-accent2 rounded-[2.5rem] p-8 text-white shadow-xl shadow-app-accent3/20 relative overflow-hidden group">
                                ...
                                <h4 className="text-xl font-black mb-4 tracking-tight">AI Executive Summary</h4>
                                <p className="text-blue-100 text-sm leading-relaxed mb-6 font-medium line-clamp-[8]">
                                    {(() => {
                                        const summary = data.business_advisor.executive_summary || data.business_advisor.smart_summary || "";
                                        const cleanSummary = summary.replace(/\[EXECUTIVE SUMMARY\]|\[STRATEGIC ACTION PLAN\]/gi, '').trim();

                                        return cleanSummary || "The intelligence engine is synthesizing data patterns to generate your executive brief.";
                                    })()}
                                </p>
                                <div className="flex items-center gap-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {data.business_advisor.recommendations.map((rec, idx) => (
                                        <div key={idx} className="flex gap-4 items-start p-4 rounded-2xl border border-app-border bg-app-bg/50 hover:bg-app-cardHover transition-colors">
                                            ...
                                            <p className="text-sm text-app-textMuted leading-snug font-medium">{rec.replace(/\*\*/g, '')}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                                ...
                                <div className="flex items-center gap-2 p-1.5 bg-white/5 border border-white/10 rounded-2xl">
                                    {['all', 'bar', 'area', 'scatter', 'pie', 'geo_map'].map(type => (
                                        <button
                                            ...
                                            className={`px-5 py-2 rounded-xl text-[11px] font-bold uppercase tracking-wider transition-all duration-300 ${filterType === type
